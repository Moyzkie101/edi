<?php
    ...
    if (isset($_POST['download'])) {
        ...
        $getLastDay_date_format = $_SESSION['getLastDay_date_format'] ?? '';


        generateDownload($conn, $database, $mainzone, $zone, $region, $status, $restrictedDate, $getLastDay_date_format);
    ...
    elseif (isset($_POST['download2'])) {
        ...
        generateDownload2($conn, $database, $mainzone, $zone, $region, $status, $restrictedDate, $getLastDay_date_format);

    }
 
    // Function to generate the download excel file
    function generateDownload($conn, $database, $mainzone, $zone, $region, $status, $restrictedDate, $getLastDay_date_format) {
        $mainzone = $_SESSION['mainzone'] ?? '';
        $zone = $_SESSION['zone'] ?? '';
        $region = $_SESSION['region'] ?? '';
        $restrictedDate = $_SESSION['restrictedDate'] ?? '';
        ...
        $dlsql = "SELECT 
            ...
            WHERE 
                p.payroll_date BETWEEN '$restrictedDate-01' AND '$restrictedDate-$getLastDay_date_format'
                AND p.description = 'midYearBonus'
                AND NOT p.payroll_date IN ('$restrictedDate-15','$restrictedDate-$getLastDay_date_format')
        ...
        if($status==='Active'){
            if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
                $filename = "EDI_MidYearBonus_Payroll-Format_Report_" . $mainzone . "_" . $region . "_" . $payroll_date_format . ".xls";
            }else{
                $filename = "EDI_MidYearBonus_Payroll-Format_Report_" . $zone . "_" . $region . "_" . $payroll_date_format . ".xls";
            }
        }else{
            if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
                $filename = "EDI_MidYearBonus_Payroll-Format_Report_" . $mainzone . "_" . $region . "_" . $payroll_date_format . "(Inactive or Pending).xls";
            }else{
                $filename = "EDI_MidYearBonus_Payroll-Format_Report_" . $zone . "_" . $region . "_" . $payroll_date_format . "(Inactive or Pending).xls";
            }
        }
    ...
    function generateDownload2($conn, $database, $mainzone, $zone, $region, $status, $restrictedDate, $getLastDay_date_format) {
        $mainzone = $_SESSION['mainzone'] ?? '';
        $zone = $_SESSION['zone'] ?? '';
        $region = $_SESSION['region'] ?? '';
        $restrictedDate = $_SESSION['restrictedDate'] ?? '';
        ...
        $dlsql = "SELECT 
            ...
            if ($mainzone === 'ALL'){
                ...
                if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
                    ...
                    $dlsql .= " AND p.zone = '$zone' AND p.region_code LIKE '%$region%' AND p.ml_matic_region NOT IN ('LNCR Showroom', 'VISMIN Showroom') ";
                }
            }
            $dlsql .= " AND (
                    CASE 
        ...
        if($mainzone === 'ALL' || $zone === 'ALL') {
            if(mysqli_num_rows($dlresult) > 0) {
                ...
                $groups = [
                    'EDI_MidYearBonus_Payroll-Format_Report_LZN_' => [],
                    'EDI_MidYearBonus_Payroll-Format_Report_NCR_' => [],
                    'EDI_MidYearBonus_Payroll-Format_Report_VIS_' => [],
                    'EDI_MidYearBonus_Payroll-Format_Report_MIN_' => [],
                    'EDI_MidYearBonus_Payroll-Format_Report_NATIONWIDE-SHOWROOM_' => [],
                ];
                while ($row = mysqli_fetch_assoc($dlresult)) {
                    ...
                    if ($row['zone'] === 'LZN' && $row['ml_matic_region'] !== 'LNCR Showroom' && $row['ml_matic_region'] !== 'VISMIN Showroom') {
                        $groups['EDI_MidYearBonus_Payroll-Format_Report_LZN_'][] = $row;
                    }
                    // Only include in NCR.xls if zone = 'NCR' and not showroom
                        ...
                        $groups['EDI_MidYearBonus_Payroll-Format_Report_NCR_'][] = $row;
                    }
                    // Only include in VISAYAS.xls if zone = 'VIS' and not showroom
                        ...
                        $groups['EDI_MidYearBonus_Payroll-Format_Report_VIS_'][] = $row;
                    }
                    // Only include in MINDANAO.xls if zone = 'MIN' and not showroom
                        ...
                        $groups['EDI_MidYearBonus_Payroll-Format_Report_MIN_'][] = $row;
                    }
                    // Only include in NATIONWIDE-SHOWROOM.xls if zone is showroom
                        ...
                        $groups['EDI_MidYearBonus_Payroll-Format_Report_NATIONWIDE-SHOWROOM_'][] = $row;
                    }
                }
                ...
                // 4. Zip the files
                $zipPath = $tmpDir . '/EDI_MidYearBonus_Payroll-Format_Report_' . $payroll_date_format . '.zip';
                $zip = new ZipArchive();
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['generate'])) {
        ...
        $sql = "SELECT 
            ...
            FROM 
                " . $database[0] . ".payroll_edi_report p 
            WHERE 
                p.payroll_date BETWEEN '$getYearMonth_date_format-01' AND '$getYearMonth_date_format-$getLastDay_date_format'
                AND p.description = 'midYearBonus'
        ...
        $_SESSION['restrictedDate'] = $getYearMonth_date_format;
        $_SESSION['getLastDay_date_format'] = $getLastDay_date_format;
...
<body>
    ...
    <center><h2>Mid Year Bonus Payroll <span style="font-size: 22px; color: red;">[EDI-Format]</span></h2></center>
        ...
        <?php if(isset($_POST['mainzone']) && $_POST['mainzone'] == 'ALL' || isset($_POST['zone']) && $_POST['zone'] == 'ALL') :?>
            <div id="showdl" style="display: none">
                <form id="exportForm" action="" method="post">
                    <input type="submit" class="download-btn" name="download2" value="Export to Excel">
                </form>
            </div>
        <?php else:?>
            <div id="showdl" style="display: none">
    ...
    <?php
        ...
        if (!$result || mysqli_num_rows($result) === 0):
            ...
            if(empty($first_row['ml_matic_status'])):
                echo "<div style='text-align: center; color: red; margin: 20px; font-size: 18px;'>
