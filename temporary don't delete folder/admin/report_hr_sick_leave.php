<?php
    ...
    if(isset($_POST['download'])) {
        ...
        $dlsql = "SELECT p.* FROM " . $database[0] . ".sick_leave p
                  WHERE 
                      p.mainzone = '$mainzone'
...
<body>
    ...
    <center><h2>Payroll Report <span style="font-size: 22px; color: red;">[HRMD-Format]</span></h2></center>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['generate'])) {
        ...
        $sql = "SELECT p.* FROM " . $database[0] . ".sick_leave p
                WHERE 
                    p.mainzone = '$mainzone'
                    AND (p.zone = '$zone' OR p.zone = 'JVIS')
                    AND p.region_code LIKE '%$region%'
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td>" . htmlspecialchars(number_format($row['sick_leave'],2)) . "</td>"; 
