<?php
    ...
    if (isset($_POST['download'])) {
        ...
        $dlsql = "WITH closed_branch_sums AS (
                    SELECT
                        region_code,
                        SUM(CASE WHEN ml_matic_status IN ('Inactive') THEN excess_pb ELSE 0 END) AS sum_excess_pb
                    ...
                    AND ml_matic_status IN ('Inactive')
                ...
                active_branch_count AS (
                    ...
                    AND NOT payroll_date IN ('$getYearMonth_date_format-15','$getYearMonth_date_format-$getLastDay_date_format')
                    AND NOT description = 'payroll'
                    AND ml_matic_status IN ('Active', 'Pending')
                    GROUP BY region_code
                )
                ...
                WHERE epr.ml_matic_status IN ('Active', 'Pending')
        ...
        if ($mainzone === 'ALL'){
            ...
            if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
                ...
                $dlsql .= " AND epr.zone = '$zone' AND epr.region_code LIKE '%$region%' AND NOT epr.ml_matic_region IN ('LNCR Showroom', 'VISMIN Showroom') ";
            }
        }
        ...
        if($mainzone === 'ALL' || $zone === 'ALL') {
            if(mysqli_num_rows($dlresult) > 0) {
                ...
                $groups = [
                    'EDI_Mid_Year_Provision_Allocation_Report_LZN_' => [],
                    'EDI_Mid_Year_Provision_Allocation_Report_NCR_' => [],
                    'EDI_Mid_Year_Provision_Allocation_Report_VIS_' => [],
                    'EDI_Mid_Year_Provision_Allocation_Report_MIN_' => [],
                    'EDI_Mid_Year_Provision_Allocation_Report_NATIONWIDE-SHOWROOM_' => [],
                ];
                ...
                $zipFilename = 'EDI_Mid_Year_Provision_Allocation_Report_' . $payroll_date_format . '.zip';
            ...
            if (mysqli_num_rows($dlresult) > 0) {
                ...
                if($status==='Active'){
                    if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
                        $filename = "EDI_Mid_Year_Provision_Allocation_Report_" . $mainzone . "_" . $region . "_" . $payroll_date_format . ".xls";
                    }else{
                        $filename = "EDI_Mid_Year_Provision_Allocation_Report_" . $zone . "_" . $region . "_" . $payroll_date_format . ".xls";
                    }
                    
                }else{
                    if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
                        $filename = "EDI_Mid_Year_Provision_Allocation_Report_" . $mainzone . "_" . $region . "_" . $payroll_date_format . "(Inactive or Pending).xls";
                    }else{
                        $filename = "EDI_Mid_Year_Provision_Allocation_Report_" . $zone . "_" . $region . "_" . $payroll_date_format . "(Inactive or Pending).xls";
                    }
                }
...
<body>
    ...
    <center><h2>Mid Year Bonus Provision Allocation Report <span style="font-size: 22px; color: red;">[EDI-Format]</span></h2></center>
    ...
    $sql = "WITH closed_branch_sums AS (
        ...
        if ($mainzone === 'ALL'){
            ...
            if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
                ...
                // Adjust the SQL query based on the selected zone
                $sql .= " AND epr.zone = '$zone' AND epr.region_code LIKE '%$region%' AND NOT epr.ml_matic_region IN ('LNCR Showroom', 'VISMIN Showroom') ";
            }
        }
        // Get the result
