...
<body>
    ...
    <center><h2>OPERATION DEDUCTION <span>[IMPORT]</span></h2></center>
...
function insertData($spreadsheet, $conn, $database, $restrictedDate, $mainzone) {
    ...
    foreach ($spreadsheet->getWorksheetIterator() as $worksheet) {
        ...
        for ($row = 5; $row <= $highestRow; ++$row) {
            ...
            $column12 = $conn->real_escape_string(floatval($worksheet->getCell('L' . $row)->getValue()));
            $column13 = $conn->real_escape_string(floatval($worksheet->getCell('M' . $row)->getValue()));
            $column14 = $conn->real_escape_string(floatval($worksheet->getCell('N' . $row)->getValue()));
            $column15 = $conn->real_escape_string(floatval($worksheet->getCell('O' . $row)->getValue()));
            $column16 = $conn->real_escape_string(floatval($worksheet->getCell('P' . $row)->getValue()));
            $column17 = $conn->real_escape_string(floatval($worksheet->getCell('Q' . $row)->getValue()));
            $column18 = $conn->real_escape_string(floatval($worksheet->getCell('R' . $row)->getValue()));
            $column19 = $conn->real_escape_string(floatval($worksheet->getCell('S' . $row)->getValue()));
            $column20 = $conn->real_escape_string(floatval($worksheet->getCell('T' . $row)->getValue()));
            $column21 = $conn->real_escape_string(floatval($worksheet->getCell('U' . $row)->getValue()));
            $column22 = $conn->real_escape_string(floatval($worksheet->getCell('V' . $row)->getValue()));
            $column23 = $conn->real_escape_string(strval($worksheet->getCell('W' . $row)->getValue()));
            ...
            $sql = "INSERT INTO " . $database[0] . ".operation_deduction (operation_date, mainzone, `zone`, region, region_code, bos_code, branch_name,
                                                        income_tax, gl_code_income_tax, sss_contributon, gl_code_sss_contribution, sss_loan, gl_code_sss_loan, pagibig_contribution, gl_code_pagibig_contribution, pagibig_loan, gl_code_pagibig_loan,
                                                        philhealth, gl_code_philhealth, coated, gl_code_coated, hmo, gl_code_hmo, canteen, gl_code_canteen, deduction_one, gl_code_deduction_one,
                                                        deduction_two, gl_code_deduction_two, ml_fund, gl_code_ml_fund, opec, gl_code_opec, over_appraisal, gl_code_over_appraisal, vpo_collection, gl_code_vpo_collection,
                                                        installment_account, gl_code_installment_account, ticket, gl_code_ticket, mobile_bill, gl_code_mobile_bill, sako, gl_code_sako, sako_savings, gl_code_sako_savings,
                                                        sheet_name, uploaded_by, uploaded_date, post_edi)

                    VALUES ('$restrictedDate', '$mainzone', '$zone_code', '$region_description', '$column23',$column1, '$column2',
                            $column3, $glCodes[0], $column4, $glCodes[1], $column5, $glCodes[2], $column6, $glCodes[3], $column7, $glCodes[4],
                            $column8, $glCodes[5], $column9, $glCodes[6], $column10, $glCodes[7], $column11, $glCodes[8], $column12, $glCodes[9],
                            $column13, $glCodes[10], $column14, $glCodes[11], $column15, $glCodes[12], $column16, $glCodes[13], $column17, $glCodes[14],
                            $column18, $glCodes[15], $column19, $glCodes[16], $column20, $glCodes[17], $column21, $glCodes[18], $column22, $glCodes[19],
...
if (isset($_POST['upload'])) {
    ...
    if ($format === 'per-branch') {
        ...
        if (move_uploaded_file($filePath, $destination)) {
            ...
            echo "<script>alert('File upload failed.'); window.location.href='import-operation-deduction.php';</script>";
            exit;
        ...
        function displayMessages($messages) {
            ...
            echo '<script>
                ...
                function exportToPDF() {
                    ...
                    payrollDateInput.name = "operation_date";
        ...
        function checkExistingRecords($conn, $database, $regionCode, $date, $mainzone) {
            $sql = "SELECT COUNT(*) as count FROM " . $database[0] . ".operation_deduction WHERE region_code = '" . $conn->real_escape_string($regionCode) . "' 
                    AND operation_date = '" . $conn->real_escape_string($date) . "' AND mainzone = '" . $conn->real_escape_string($mainzone) . "'";
        ...
        foreach ($spreadsheet->getAllSheets() as $sheet) {
            ...
            for ($row = $startRow; $row <= $endRow; $row++) {
                ...
                if (checkExistingRecords($conn, $database, $cellValues['W'], $_POST['restricted-date'], $check_mainzone)) {
                    $region_description = $regionDescriptions[$cellValues['W']] ?? 'Unknown region';
                    $messages[] = [
                        'type' => 'error',
                        'withButton' => 'true',
                        'sheet' => $sheetName,
                ...
                $dbData = getDatabaseData($conn, $database, $cellValues['A'], $cellValues['W']);
                if (!$dbData) {
                    $region_description = $regionDescriptions[$cellValues['W']] ?? 'Unknown region';
                    $messages[] = [
                        'type' => 'error',
                ...
                if (!in_array($cellValues['W'], $validRegionCodes)) {
                    $region_description = $regionDescriptions[$cellValues['W']] ?? 'Unknown region';
                    $messages[] = [
                        'type' => 'error',
                        'withButton' => 'false',
                ...
                if (checkStatus($conn, $database, $cellValues['A'], $cellValues['W'])) {
                    $region_description = $regionDescriptions[$cellValues['W']] ?? 'Unknown region';
                    $messages[] = [
                        'type' => 'error',
                        'withButton' => 'false',
        ...
        foreach ($codeDetails as $zone => $branchCodes) {
            foreach ($branchCodes as $branchCode => $details) {
                if (count($details) > 1) {  // Duplicate found
                    $region_description = $regionDescriptions[$details[0]['W']] ?? 'Unknown region';
        ...
        if (empty($messages)) {
            ...
            echo "<script>
                    if (confirm('File is ready to upload. Do you want to continue?')) {
                        window.location.href = 'import-operation-deduction.php?proceed=true';
                    } else {
                        window.location.href = 'import-operation-deduction.php';
                    }
                </script>";
...
if (isset($_GET['proceed']) && $_GET['proceed'] === 'true') {
    ...
    if ($format === 'per-branch') {
        ...
        if ($insertSuccess) {
            echo "<script>alert('Data successfully loaded.'); window.location.href='import-operation-deduction.php';</script>";
        } else {
            echo "<script>alert('Failed to upload.'); window.location.href='import-operation-deduction.php';</script>";
        }
    }
}
...
if (isset($_POST['overrideData'])) {
    ...
    foreach ($regionCodesToDelete as $regionCode) {
        ...
        $sql = "SELECT DISTINCT post_edi FROM " . $database[0] . ".operation_deduction WHERE region_code = '" . $conn->real_escape_string($regionCode) . "' 
                AND operation_date = '" . $conn->real_escape_string($date) . "' AND mainzone = '" . $conn->real_escape_string($mainzone) . "'";
        ...
        if ($row_resultPost['post_edi'] === 'pending') {
            ...
            foreach ($regionCodesToDelete as $regionCode) {
                ...
                $sql = "DELETE FROM " . $database[0] . ".operation_deduction WHERE region_code = '" . $conn->real_escape_string($regionCode) . "' 
                        AND operation_date = '" . $conn->real_escape_string($date) . "' AND mainzone = '" . $conn->real_escape_string($mainzone) . "'";
                $conn->query($sql);
            }
            ...
            if ($insertSuccess) {
                ...
                echo "<script>alert('Insertion Failed.'); window.location.href='import-operation-deduction.php';</script>";
            }
            ...
            echo "<script>alert('Opps! Unable to Override. Data Already Posted.'); window.location.href='import-operation-deduction.php';</script>";
        }
    }
}
