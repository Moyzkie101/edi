...
<body>
    ...
    <center><h2>SICK LEAVE CONVERSION <span>[IMPORT]</span></h2></center>
...
function insertData($spreadsheet, $conn, $database, $restrictedDate, $mainzone) {
    ...
    foreach ($spreadsheet->getWorksheetIterator() as $worksheet) {
        ...
        for ($row = 5; $row <= $highestRow; ++$row) {
            ...
            $sql = "INSERT INTO " . $database[0] . ".sick_leave (
                                    payroll_date, 
                                    mainzone, 
                                    zone, 
                                    region, 
...
if (isset($_POST['upload'])) {
        ...
        if (move_uploaded_file($filePath, $destination)) {
            ...
            echo "<script>alert('File upload failed.'); window.location.href='import-sick-leave.php';</script>";
            exit;
        ...
        function displayMessages($messages) {
            ...
            echo '<script>
                ...
                function exportToPDF() {
                    ...
                    form.method = "post";
                    form.action = "../../models/generate/pdf/payroll_export_pdf.php";
    ...
    function checkExistingRecords($conn, $database, $regionCode, $date, $mainzone) {
        $sql = "SELECT COUNT(*) as count FROM " . $database[0] . ".sick_leave WHERE region_code = '" . $conn->real_escape_string($regionCode) . "' 
    ...
    if (empty($messages)) {
        ...
        echo "<script>
                if (confirm('File is ready to upload. Do you want to continue?')) {
                    window.location.href = 'import-sick-leave.php?proceed=true';
                } else {
                    window.location.href = 'import-sick-leave.php';
                }
            </script>";
...
if (isset($_GET['proceed']) && $_GET['proceed'] === 'true') {
    ...
    if ($insertSuccess) {
        echo "<script>alert('Data successfully loaded.'); window.location.href='import-sick-leave.php';</script>";
    } else {
        echo "<script>alert('Failed to upload.'); window.location.href='import-sick-leave.php';</script>";
    }
}
...
if (isset($_POST['overrideData'])) {
    ...
    foreach ($regionCodesToDelete as $regionCode) {
        ...
        $sql = "SELECT DISTINCT post_edi FROM " . $database[0] . ".sick_leave WHERE region_code = '" . $conn->real_escape_string($regionCode) . "' 
                AND payroll_date = '" . $conn->real_escape_string($date) . "' AND mainzone = '" . $conn->real_escape_string($mainzone) . "'";
        ...
        if ($row_resultPost['post_edi'] === 'pending') {
            ...
            foreach ($regionCodesToDelete as $regionCode) {
                ...
                $sql = "DELETE FROM " . $database[0] . ".sick_leave WHERE region_code = '" . $conn->real_escape_string($regionCode) . "' 
                        AND payroll_date = '" . $conn->real_escape_string($date) . "' AND mainzone = '" . $conn->real_escape_string($mainzone) . "'";
                $conn->query($sql);
            }
            ...
            if ($insertSuccess) {
                ...
                echo "<script>alert('Insertion Failed.'); window.location.href='import-sick-leave.php';</script>";
            }
            ...
            echo "<script>alert('Opps! Unable to Override. Data Already Posted.'); window.location.href='import-sick-leave.php';</script>";
        }
    }
