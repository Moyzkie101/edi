    ...
    foreach ($spreadsheet->getWorksheetIterator() as $worksheet) {
        ...
        for ($row = 5; $row <= $highestRow; ++$row) {
            ...
            $sql = "INSERT INTO " . $database[0] . ".payroll (payroll_date, mainzone, zone, region, region_code, bos_code, branch_name, basic_pay_regular, gl_code_basic_pay_regular, 
                        ...
                        '$column24', '$column25', '$column26', 'payroll', 'pending')";
...
if (isset($_POST['upload'])) {
    ...
    function checkExistingRecords($conn, $database, $regionCode, $date, $mainzone) {
        $sql = "SELECT COUNT(*) as count FROM " . $database[0] . ".payroll WHERE region_code = '" . $conn->real_escape_string($regionCode) . "' 
                AND payroll_date = '" . $conn->real_escape_string($date) . "' AND mainzone = '" . $conn->real_escape_string($mainzone) . "' and description = 'payroll'";
