    ...
    foreach ($spreadsheet->getWorksheetIterator() as $worksheet) {
        ...
        for ($row = 5; $row <= $highestRow; ++$row) {
            ...
            $sql = "INSERT INTO " . $database[0] . ".payroll (payroll_date, mainzone, zone, region, region_code, bos_code, branch_name, basic_pay_regular, gl_code_basic_pay_regular, 
                        ...
                        '$column24', '$column25', '$column26', 'Sick-Leave', 'pending')";
...
if (isset($_POST['upload'])) {
    ...
    try {
        ...
        echo "<script>alert('" . addslashes($e->getMessage()) . "'); window.location.href='import-sick-leave.php';</script>";
        exit;
    }
        ...
        function displayMessages($messages) {
            ...
            echo "<table>
                <thead>
                    <tr>
                        <center>
                            <th colspan='7' style='border: none;'>Sick Leave Conversion Report</th>
                        </center>
                    </tr>
                    <tr>
    ...
    function checkExistingRecords($conn, $database, $regionCode, $date, $mainzone) {
        $sql = "SELECT COUNT(*) as count FROM " . $database[0] . ".payroll WHERE region_code = '" . $conn->real_escape_string($regionCode) . "' 
                AND payroll_date = '" . $conn->real_escape_string($date) . "' AND mainzone = '" . $conn->real_escape_string($mainzone) . "' and description = 'Sick-Leave'";
...
if (isset($_GET['proceed']) && $_GET['proceed'] === 'true') {
    ...
    if ($filePath === null) {
        echo "<script>alert('The uploaded file is no longer available.'); window.location.href='import-sick-leave.php';</script>";
        exit;
    }
