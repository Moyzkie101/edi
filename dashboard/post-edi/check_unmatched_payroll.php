<?php
    ...
    /**
     * Finds payroll rows for a given mainzone/date that fail the
     * branch_profile join used by both post-edi_payroll.php and
     * unpost-edi_payroll.php. Rows returned here will NEVER show up
     * in either page's preview, no matter what post_edi says.
     */
    function fetchUnmatchedPayroll($conn, $database, string $mainzone, string $restrictedDate): array
    {
        $sql = "SELECT
                ...
                ORDER BY p.region, p.branch_name";
    ...
    /**
     * Given one unmatched payroll row, works out WHY it doesn't match
...
<body>
    ...
    <center><h2>PAYROLL <span>[UNMATCHED BRANCH CHECK]</span></h2></center>

    <div class="banner-warning" style="margin-left:auto;margin-right:auto;max-width:900px;">
        This lists imported payroll rows that don't match any <code>branch_profile</code> record —
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['generate'])) {
        ...
        if ($mainzone === '' || $restrictedDate === '') {
            echo "<div class='banner-warning' style='margin-left:auto;margin-right:auto;max-width:900px;'>Please select a Mainzone and Payroll date.</div>";
        } else {
            $result = fetchUnmatchedPayroll($conn, $database, $mainzone, $restrictedDate);
            ...
            if ($result['error'] !== null) {
                ...
                echo "<div class='banner-ok' style='margin-left:auto;margin-right:auto;max-width:900px;'>No unmatched rows found — every payroll row for this mainzone/date joins cleanly to branch_profile.</div>";
            } else {
