<?php
    ...
    /**
     * Finds remitance rows for a given mainzone/date that fail the
     * branch_profile join used by both post-edi_remittance-new.php and
     * unpost-edi_remittance-new.php. Rows returned here will NEVER show
     * up in either page's preview, no matter what post_edi says.
     */
    function fetchUnmatchedRemitance($conn, $database, string $mainzone, string $restrictedDate): array
    {
        $sql = "SELECT
                    r.id, r.bos_code, r.branch_name, r.region, r.region_code, r.zone,
                    r.mainzone, r.remitance_date, r.post_edi, r.remitance_format_type,
                    r.uploaded_by, r.uploaded_date
                ...
                ORDER BY r.region, r.branch_name";

        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            return ['error' => $conn->error, 'rows' => []];
        }
        $format = REMIT_FORMAT_TYPE;
        $stmt->bind_param('sss', $mainzone, $restrictedDate, $format);
        $stmt->execute();
    ...
    /**
     * Given one unmatched remitance row, works out WHY it doesn't match
...
<body>
    ...
    <center><h2>REMITTANCE NEW <span>[UNMATCHED BRANCH CHECK]</span></h2></center>

    <div class="banner-warning" style="margin-left:auto;margin-right:auto;max-width:900px;">
        This lists imported remittance rows that don't match any <code>branch_profile</code> record —
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['generate'])) {
        ...
        if ($mainzone === '' || $restrictedDate === '') {
            echo "<div class='banner-warning' style='margin-left:auto;margin-right:auto;max-width:900px;'>Please select a Mainzone and Remittance date.</div>";
        } else {
            $result = fetchUnmatchedRemitance($conn, $database, $mainzone, $restrictedDate);
            ...
            if ($result['error'] !== null) {
                ...
                echo "<div class='banner-ok' style='margin-left:auto;margin-right:auto;max-width:900px;'>No unmatched rows found — every remittance row for this mainzone/date joins cleanly to branch_profile.</div>";
            } else {
                echo "<div style='margin:10px 20px;color:red;font-weight:bold;'>Total Unmatched Rows: " . count($result['rows']) . "</div>";
                echo "<div class='table-container'>";
                echo "<table>";
                echo "<thead><tr>";
                echo "<th>Status</th><th>Bos Code</th><th>Branch Name</th><th>Zone</th><th>Region</th><th>Region Code</th>";
                echo "<th>Format Type</th><th>Uploaded By</th><th>Uploaded Date</th><th>Why it doesn't match</th>";
                echo "</tr></thead><tbody>";
                ...
                foreach ($result['rows'] as $row) {
                    ...
                    echo "<td>" . htmlspecialchars((string) $row['remitance_format_type']) . "</td>";
