<?php
    ...
    const DESCRIPTION_13TH = '13thMonth';
    const EXCLUDED_DESCRIPTIONS_13TH = ['payroll', 'midYearBonus', 'Sick-Leave'];

    function buildFilterClause13th(string $mainzone, string $zone, string $region, string $restrictedDate): array
    {
        ...
        if ($isShowroom) {
            ...
            $types  = 'ssss';
            $values = [$mainzone, $restrictedDate, $zone, DESCRIPTION_13TH];
            ...
            $where = "bp.mainzone = ?
                      ...
                      AND p.remarks IS NULL";
            $types  = 'ssss';
            $values = [$mainzone, $zone, $restrictedDate, DESCRIPTION_13TH];
        ...
        $placeholders = implode(',', array_fill(0, count(EXCLUDED_DESCRIPTIONS_13TH), '?'));
        $where .= " AND p.description NOT IN ($placeholders)";
        foreach (EXCLUDED_DESCRIPTIONS_13TH as $d) {
            $types   .= 's';
            $values[] = $d;
        }
    ...
    function fetchPostedPreview13th($conn, $database, $mainzone, $zone, $region, $restrictedDate)
    {
        [$where, $types, $values] = buildFilterClause13th($mainzone, $zone, $region, $restrictedDate);
        $join = sprintf(JOIN_CLAUSE_13TH, $database[0], $database[1]);
    ...
    function unpostData13th($conn, $database, $mainzone, $zone, $region, $restrictedDate): array
    {
        [$where, $types, $values] = buildFilterClause13th($mainzone, $zone, $region, $restrictedDate);
        ...
        try {
            ...
            $reportTypes = 'sss';
            $reportValues = [$mainzone, $restrictedDate, DESCRIPTION_13TH];
            ...
            $conn->rollback();
            error_log('Unpost EDI (13th month) failed: ' . $e->getMessage());
            return ['success' => false, 'error' => $e->getMessage()];
        }
    }

    if (isset($_GET['proceed']) && $_GET['proceed'] === 'true') {
        $mainzone       = $_SESSION['unpost_13th_mainzone'] ?? '';
        $zone           = $_SESSION['unpost_13th_zone'] ?? '';
        $region         = $_SESSION['unpost_13th_region'] ?? '';
        $restrictedDate = $_SESSION['unpost_13th_restrictedDate'] ?? '';
        ...
        if ($mainzone === '' || $zone === '' || $restrictedDate === '') {
            ...
            $outcome = unpostData13th($conn, $database, $mainzone, $zone, $region, $restrictedDate);

            if ($outcome['success'] && $outcome['reverted_payroll_rows'] > 0) {
                $_SESSION['swal_message'] = [
                    'title' => 'Success!',
                    'text'  => 'Unposted ' . $outcome['reverted_payroll_rows'] . ' 13th Month record(s). You can re-import or re-post once corrected.',
                    'icon'  => 'success',
                ];
        ...
        header('Location: unpost-edi_13th-month.php');
        exit();
    }
...
<body>
    ...
    <center><h2>13TH MONTH <span>[UNPOST EDI]</span></h2></center>

    <div class="banner-warning" style="margin-left:auto;margin-right:auto;max-width:900px;">
        Unposting reverts posted 13th Month records back to <strong>pending</strong> and removes their EDI
...
<script>
    function unpostEdi() {
        Swal.fire({
            title: 'Are you sure?',
            text: 'This will revert the selected posted 13th Month data back to pending and remove it from the EDI report.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                window.location.href = 'unpost-edi_13th-month.php?proceed=true';
            } else {
                window.location.href = 'unpost-edi_13th-month.php';
            }
        });
    }
</script>
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['generate'])) {
    ...
    $_SESSION['unpost_13th_mainzone']       = $mainzone;
    $_SESSION['unpost_13th_zone']           = $zone;
    $_SESSION['unpost_13th_region']         = $region;
    $_SESSION['unpost_13th_restrictedDate'] = $restrictedDate;
    ...
    if ($mainzone === '' || $zone === '' || $restrictedDate === '') {
        ...
        $preview = fetchPostedPreview13th($conn, $database, $mainzone, $zone, $region, $restrictedDate);
