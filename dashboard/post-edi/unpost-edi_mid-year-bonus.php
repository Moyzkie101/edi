<?php
    ...
    const DESCRIPTION_MYB = 'midYearBonus';
    const EXCLUDED_DESCRIPTIONS_MYB = ['payroll', '13thMonth', 'Sick-Leave'];

    function buildFilterClauseMYB(string $mainzone, string $zone, string $region, string $restrictedDate): array
    {
        ...
        if ($isShowroom) {
            ...
            $types  = 'ssss';
            $values = [$mainzone, $restrictedDate, $zone, DESCRIPTION_MYB];
            ...
            $where = "bp.mainzone = ?
                      ...
                      AND p.remarks IS NULL";
            $types  = 'ssss';
            $values = [$mainzone, $zone, $restrictedDate, DESCRIPTION_MYB];
        ...
        $placeholders = implode(',', array_fill(0, count(EXCLUDED_DESCRIPTIONS_MYB), '?'));
        $where .= " AND p.description NOT IN ($placeholders)";
        foreach (EXCLUDED_DESCRIPTIONS_MYB as $d) {
            $types   .= 's';
            $values[] = $d;
        }
    ...
    function fetchPostedPreviewMYB($conn, $database, $mainzone, $zone, $region, $restrictedDate)
    {
        [$where, $types, $values] = buildFilterClauseMYB($mainzone, $zone, $region, $restrictedDate);
        $join = sprintf(JOIN_CLAUSE_MYB, $database[0], $database[1]);
    ...
    function unpostDataMYB($conn, $database, $mainzone, $zone, $region, $restrictedDate): array
    {
        [$where, $types, $values] = buildFilterClauseMYB($mainzone, $zone, $region, $restrictedDate);
        ...
        try {
            $reportWhere = "mainzone = ? AND payroll_date = ? AND `description` = ?";
            $reportTypes = 'sss';
            $reportValues = [$mainzone, $restrictedDate, DESCRIPTION_MYB];
            ...
            $conn->rollback();
            error_log('Unpost EDI (mid year bonus) failed: ' . $e->getMessage());
            return ['success' => false, 'error' => $e->getMessage()];
        }
    }

    if (isset($_GET['proceed']) && $_GET['proceed'] === 'true') {
        $mainzone       = $_SESSION['unpost_myb_mainzone'] ?? '';
        $zone           = $_SESSION['unpost_myb_zone'] ?? '';
        $region         = $_SESSION['unpost_myb_region'] ?? '';
        $restrictedDate = $_SESSION['unpost_myb_restrictedDate'] ?? '';
        ...
        if ($mainzone === '' || $zone === '' || $restrictedDate === '') {
            ...
            $outcome = unpostDataMYB($conn, $database, $mainzone, $zone, $region, $restrictedDate);

            if ($outcome['success'] && $outcome['reverted_payroll_rows'] > 0) {
                $_SESSION['swal_message'] = [
                    'title' => 'Success!',
                    'text'  => 'Unposted ' . $outcome['reverted_payroll_rows'] . ' Mid Year Bonus record(s). You can re-import or re-post once corrected.',
                    'icon'  => 'success',
                ];
        ...
        header('Location: unpost-edi_mid-year-bonus.php');
        exit();
    }
...
<body>
    ...
    <center><h2>MID YEAR BONUS <span>[UNPOST EDI]</span></h2></center>

    <div class="banner-warning" style="margin-left:auto;margin-right:auto;max-width:900px;">
        Unposting reverts posted Mid Year Bonus records back to <strong>pending</strong> and removes their EDI
...
<script>
    function unpostEdi() {
        Swal.fire({
            title: 'Are you sure?',
            text: 'This will revert the selected posted Mid Year Bonus data back to pending and remove it from the EDI report.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                window.location.href = 'unpost-edi_mid-year-bonus.php?proceed=true';
            } else {
                window.location.href = 'unpost-edi_mid-year-bonus.php';
            }
        });
    }
</script>
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['generate'])) {
    ...
    $_SESSION['unpost_myb_mainzone']       = $mainzone;
    $_SESSION['unpost_myb_zone']           = $zone;
    $_SESSION['unpost_myb_region']         = $region;
    $_SESSION['unpost_myb_restrictedDate'] = $restrictedDate;

    if ($mainzone === '' || $zone === '' || $restrictedDate === '') {
        echo "<div class='banner-warning' style='margin-left:auto;margin-right:auto;max-width:900px;'>Please fill in Mainzone, Zone, and Payroll date.</div>";
    } else {
        $preview = fetchPostedPreviewMYB($conn, $database, $mainzone, $zone, $region, $restrictedDate);
