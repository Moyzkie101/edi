<?php
    ...
    function generateDownload2($conn, $database, $mainzone, $zone, $region, $restrictedDate) {
        ...
        $dlsql = " SELECT 
			...
			if($status==='Active'){
				if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
					$filename = "EDI_13th_Month_Payroll_Report_" . $mainzone . "_" . $region . "_" . $restrictedDate . "_RECON.xls";
				}else{
					$filename = "EDI_13th_Month_Payroll_Report_" . $zone . "_" . $region . "_" . $restrictedDate . "_RECON.xls";
				}
			}else{
				if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
					$filename = "EDI_13th_Month_Payroll_Report_" . $mainzone . "_" . $region . "_" . $restrictedDate . "(Inactive or Pending)_RECON.xls";
				}else{
					$filename = "EDI_13th_Month_Payroll_Report_" . $zone . "_" . $region . "_" . $restrictedDate . "(Inactive or Pending)_RECON.xls";
				}
			}
            ...
            // $filename = "EDI_13th_Month_Payroll_Report_" . $zone . "_" . $region . "_" . $restrictedDate . ".csv";
            // header('Content-Disposition: attachment; filename="' . $filename . '"');
            // header('Cache-Control: max-age=0');
    ...
    function generateDownload($conn, $database, $mainzone, $zone, $region, $restrictedDate) {
        ...
        if($mainzone === 'ALL' || $zone === 'ALL') {
            if(mysqli_num_rows($dlresult) > 0) {
                ...
                $groups = [
                    'EDI_13th_Month_Payroll_Report_LZN_' => [],
                    'EDI_13th_Month_Payroll_Report_NCR_' => [],
                    'EDI_13th_Month_Payroll_Report_VIS_' => [],
                    'EDI_13th_Month_Payroll_Report_MIN_' => [],
                    'EDI_13th_Month_Payroll_Report_NATIONWIDE-SHOWROOM_' => [],
                ];
                while ($row = mysqli_fetch_assoc($dlresult)) {
                    ...
                    if ($row['zone'] === 'LZN' && $row['ml_matic_region'] !== 'LNCR Showroom' && $row['ml_matic_region'] !== 'VISMIN Showroom') {
                        $groups['EDI_13th_Month_Payroll_Report_LZN_'][] = $row;
                    }
                    // Only include in NCR.xls if zone = 'NCR' and not showroom
                        ...
                        $groups['EDI_13th_Month_Payroll_Report_NCR_'][] = $row;
                    }
                    // Only include in VISAYAS.xls if zone = 'VIS' and not showroom
                        ...
                        $groups['EDI_13th_Month_Payroll_Report_VIS_'][] = $row;
                    }
                    // Only include in MINDANAO.xls if zone = 'MIN' and not showroom
                        ...
                        $groups['EDI_13th_Month_Payroll_Report_MIN_'][] = $row;
                    }
                    // Only include in NATIONWIDE-SHOWROOM.xls if zone is showroom
                        ...
                        $groups['EDI_13th_Month_Payroll_Report_NATIONWIDE-SHOWROOM_'][] = $row;
                    }
                }
                ...
                // 4. Zip the files
                $zipPath = $tmpDir . '/EDI_13th_Month_Payroll_Report_' . $restrictedDate . '.zip';
                $zip = new ZipArchive();
            ...
            if(mysqli_num_rows($dlresult) > 0) {
                ...
                if($status==='Active'){
                    if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
                        $filename = "EDI_13th_Month_Payroll_Report_" . $mainzone . "_" . $region . "_" . $restrictedDate . ".xls";
                    }else{
                        $filename = "EDI_13th_Month_Payroll_Report_" . $zone . "_" . $region . "_" . $restrictedDate . ".xls";
                    }
                }else{
                    if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
                        $filename = "EDI_13th_Month_Payroll_Report_" . $mainzone . "_" . $region . "_" . $restrictedDate . "(Inactive or Pending).xls";
                    }else{
                        $filename = "EDI_13th_Month_Payroll_Report_" . $zone . "_" . $region . "_" . $restrictedDate . "(Inactive or Pending).xls";
                    }
                }
                header('Content-Disposition: attachment; filename="' . $filename . '"');
                header('Cache-Control: max-age=0');
...
<html lang="en">
    ...
    <body>
        ...
        <center><h2>13th Month Report <span>[EDI - Payroll Format]</span></h2></center>
