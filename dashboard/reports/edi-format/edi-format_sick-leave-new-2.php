...
// Function to generate the download csv file
...
ORDER BY new_region ASC";
    ...
    // Fetch all result rows into an array
    $dlRows = [];
    ...
    if (count($dlRows) > 0) {
        ...
        // Header row
        $csvData[] = ['Zone', 'Region', 'BranchID', 'Branch', 'GLCode', 'desc', 'Amount', 'Category'];
        ...
        if ($status === 'Active') {
            if ($mainzone === 'ALL' || $zone === 'ALL') {
                if ($restrictedDate === $endDate) {
                    $filename = "EDI_Sick-Leave_conversion_Report_NATIONWIDE_" . $restrictedDate_raw . "_NEW-FORMAT-2.csv";
                } else {
                    $filename = "EDI_Sick-Leave_conversion_Report_NATIONWIDE_from_" . $restrictedDate_raw . "_to_" . $endDate_raw . "_NEW-FORMAT-2.csv";
                }
            } else {
                if ($restrictedDate === $endDate) {
                    if ($zone === 'VISMIN Showroom' || $zone === 'LNCR Showroom') {
                        $filename = "EDI_Sick-Leave_conversion_Report_" . $mainzone . "_" . $restrictedDate_raw . "_NEW-FORMAT-2.csv";
                    } else {
                        $filename = "EDI_Sick-Leave_conversion_Report_" . $zone . "_" . $region . "_" . $restrictedDate_raw . "_NEW-FORMAT-2.csv";
                    }
                } else {
                    if ($zone === 'VISMIN Showroom' || $zone === 'LNCR Showroom') {
                        $filename = "EDI_Sick-Leave_conversion_Report_" . $mainzone . "_from_" . $restrictedDate_raw . "_to_" . $endDate_raw . "_NEW-FORMAT-2.csv";
                    } else {
                        $filename = "EDI_Sick-Leave_conversion_Report_" . $zone . "_" . $region . "_from_" . $restrictedDate_raw . "_to_" . $endDate_raw . "_NEW-FORMAT-2.csv";
                    }
                }
            }
        } else {
            if ($mainzone === 'ALL' || $zone === 'ALL') {
                if ($restrictedDate === $endDate) {
                    $filename = "EDI_Sick-Leave_conversion_Report_NATIONWIDE_" . $restrictedDate_raw . "_NEW-FORMAT-(Inactive or Pending)-2.csv";
                } else {
                    $filename = "EDI_Sick-Leave_conversion_Report_NATIONWIDE_from_" . $restrictedDate_raw . "_to_" . $endDate_raw . "_NEW-FORMAT-(Inactive or Pending)-2.csv";
                }
            } else {
                if ($restrictedDate === $endDate) {
                    if ($zone === 'VISMIN Showroom' || $zone === 'LNCR Showroom') {
                        $filename = "EDI_Sick-Leave_conversion_Report_" . $mainzone . "_" . $restrictedDate_raw . "_NEW-FORMAT-(Inactive or Pending)-2.csv";
                    } else {
                        $filename = "EDI_Sick-Leave_conversion_Report_" . $zone . "_" . $region . "_" . $restrictedDate_raw . "_NEW-FORMAT-(Inactive or Pending)-2.csv";
                    }
                } else {
                    if ($zone === 'VISMIN Showroom' || $zone === 'LNCR Showroom') {
                        $filename = "EDI_Sick-Leave_conversion_Report_" . $mainzone . "_from_" . $restrictedDate_raw . "_to_" . $endDate_raw . "_NEW-FORMAT-(Inactive or Pending)-2.csv";
                    } else {
                        $filename = "EDI_Sick-Leave_conversion_Report_" . $zone . "_" . $region . "_from_" . $restrictedDate_raw . "_to_" . $endDate_raw . "_NEW-FORMAT-(Inactive or Pending)-2.csv";
                    }
                }
            }
        }
        ...
        // Output CSV
        // Output CSV with UTF-8 BOM for proper Excel display
header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="' . $filename . '"');
        header('Cache-Control: max-age=0');

        $output = fopen('php://output', 'w');
// Add UTF-8 BOM so Excel recognizes it properly
...
<head>
    ...
    <style>
        ...
        select {
            ...
            appearance: none;
            /* Remove default arrow in most modern browsers */
            color: #F14A51;
        }
        ...
        .table-container {
            ...
            /* Adjust max-height as needed based on your layout */
            margin: 20px;
...
<body>
    ...
    <center>
        <h2>Sick Leave Conversion Report <span>[EDI-Format NEW 2]</span></h2>
    </center>
    ...
    <div class="import-file">
        ...
        <div id="showdl" style="display: none">
            <form id="exportForm" action="" method="post">
                <input type="submit" class="download-btn" name="download" value="Export to CSV">
            </form>
        </div>
    </div>
...
p.region;";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if ($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom') {
                echo "<td style='background-color: $color; font-weight: $bold'>" . htmlspecialchars('JEW') . "</td>";
            } else {
