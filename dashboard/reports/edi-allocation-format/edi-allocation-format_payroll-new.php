<?php
    ...
    function generateDownload($conn, $database, $mainzone, $zone, $region, $restrictedDate) {
        ...
        $dlsql="WITH closed_branch_sums AS (
            SELECT
                region_code,
                SUM(CASE WHEN ml_matic_status $status_raw THEN basic_pay_regular ELSE 0 END) AS sum_basic_pay_regular,
                SUM(CASE WHEN ml_matic_status $status_raw THEN basic_pay_trainee ELSE 0 END) AS sum_basic_pay_trainee,
                SUM(CASE WHEN ml_matic_status $status_raw THEN allowances ELSE 0 END) AS sum_allowances,
                SUM(CASE WHEN ml_matic_status $status_raw THEN bm_allowance ELSE 0 END) AS sum_bm_allowance,
                SUM(CASE WHEN ml_matic_status $status_raw THEN overtime_regular ELSE 0 END) AS sum_overtime_regular,
                SUM(CASE WHEN ml_matic_status $status_raw THEN overtime_trainee ELSE 0 END) AS sum_overtime_trainee,
                SUM(CASE WHEN ml_matic_status $status_raw THEN cola ELSE 0 END) AS sum_cola,
                SUM(CASE WHEN ml_matic_status $status_raw THEN excess_pb ELSE 0 END) AS sum_excess_pb,
                SUM(CASE WHEN ml_matic_status $status_raw THEN other_income ELSE 0 END) AS sum_other_income,
                SUM(CASE WHEN ml_matic_status $status_raw THEN salary_adjustment ELSE 0 END) AS sum_salary_adjustment,
                SUM(CASE WHEN ml_matic_status $status_raw THEN graveyard ELSE 0 END) AS sum_graveyard,
                SUM(CASE WHEN ml_matic_status $status_raw THEN late_regular ELSE 0 END) AS sum_late_regular,
                SUM(CASE WHEN ml_matic_status $status_raw THEN late_trainee ELSE 0 END) AS sum_late_trainee,
                SUM(CASE WHEN ml_matic_status $status_raw THEN leave_regular ELSE 0 END) AS sum_leave_regular,
                SUM(CASE WHEN ml_matic_status $status_raw THEN leave_trainee ELSE 0 END) AS sum_leave_trainee,
                SUM(CASE WHEN ml_matic_status $status_raw THEN all_other_deductions ELSE 0 END) AS sum_all_other_deductions,
                SUM(CASE WHEN ml_matic_status $status_raw THEN total ELSE 0 END) AS sum_total
        ...
        ON	epr.branch_code = nd.code
        AND (
            (nd.ml_matic_region IN ('VISMIN Showroom', 'LNCR Showroom') 
                AND epr.ml_matic_region = nd.ml_matic_region)
            OR 
            (epr.zone = nd.zone AND epr.region_code = nd.region_code)
        )
        WHERE epr.ml_matic_status = 'Active'
        AND epr.payroll_date = '$restrictedDate'
        AND epr.description = 'payroll'
        AND NOT epr.description IN ('Sick-Leave', '13thMonth', 'midYearBonus')
        ...
        $dlsql .= " GROUP BY 
            ...
            epr.ml_matic_region,
            nd.branch_id,
            cbs.sum_basic_pay_regular,
            cbs.sum_basic_pay_trainee,
            cbs.sum_allowances,
            cbs.sum_bm_allowance,
            cbs.sum_overtime_regular,
            cbs.sum_overtime_trainee,
            cbs.sum_cola,
            cbs.sum_excess_pb,
            cbs.sum_other_income,
            cbs.sum_salary_adjustment,
            cbs.sum_graveyard,
            
            cbs.sum_late_regular,
            cbs.sum_late_trainee,
            cbs.sum_leave_regular,
            cbs.sum_leave_trainee,
            cbs.sum_all_other_deductions,
            epr.no_of_branch_employee,
            epr.no_of_employees_allocated,
            epr.cost_center,
            abc.active_count,
        ...
        if(mysqli_num_rows($dlresult) > 0) {
            ...
            if($mainzone !== 'ALL' || $zone !== 'ALL'){
                if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
                    if(empty($region)){
                        $filename = "EDI_Allocation".$excel_filename."_Payroll_Report_" . $mainzone . "_SHOWROOM_". $restrictedDate_raw ."_NEW-FORMAT.xls";
                    } else {
                        $filename = "EDI_Allocation".$excel_filename."_Payroll_Report_" . $mainzone . "_SHOWROOM_". $region ."_" . $restrictedDate_raw ."_NEW-FORMAT.xls";
                    }
                }else{
                    if(empty($region)){
                        $filename = "EDI_Allocation".$excel_filename."_Payroll_Report_" . $zone ."_". $restrictedDate_raw ."_NEW-FORMAT.xls";
                    }else{
                        $filename = "EDI_Allocation".$excel_filename."_Payroll_Report_" . $zone ."_". $region ."_". $restrictedDate_raw ."_NEW-FORMAT.xls";
                    }
                }
            }else{
                $filename = 'EDI_Allocation'.$excel_filename.'_Payroll_Report_NATIONWIDE_'. $restrictedDate_raw .'_NEW-FORMAT.xls';
            }
            ...
            //         $groups = [
            //             'EDI_Allocation'.$excel_filename.'_Payroll_Report_LZN_' => [],
            //             'EDI_Allocation'.$excel_filename.'_Payroll_Report_NCR_' => [],
            //             'EDI_Allocation'.$excel_filename.'_Payroll_Report_VIS_' => [],
            //             'EDI_Allocation'.$excel_filename.'_Payroll_Report_MIN_' => [],
            //             'EDI_Allocation'.$excel_filename.'_Payroll_Report_NATIONWIDE-SHOWROOM_' => []
            //         ];
            ...
            //                 $groups['EDI_Allocation'.$excel_filename.'_Payroll_Report_LZN_'][] = $row;
            //             } elseif ($row['zone'] == 'NCR' && !in_array($row['ml_matic_region'], ['LNCR Showroom', 'VISMIN Showroom'])) {
            //                 $groups['EDI_Allocation'.$excel_filename.'_Payroll_Report_NCR_'][] = $row;
            //             } elseif ($row['zone'] == 'VIS' && !in_array($row['ml_matic_region'], ['LNCR Showroom', 'VISMIN Showroom'])) {
            //                 $groups['EDI_Allocation'.$excel_filename.'_Payroll_Report_VIS_'][] = $row;
            //             } elseif ($row['zone'] == 'MIN' && !in_array($row['ml_matic_region'], ['LNCR Showroom', 'VISMIN Showroom'])) {
            //                 $groups['EDI_Allocation'.$excel_filename.'_Payroll_Report_MIN_'][] = $row;
            //             } elseif (in_array($row['ml_matic_region'], ['LNCR Showroom', 'VISMIN Showroom'])) {
            //                 $groups['EDI_Allocation'.$excel_filename.'_Payroll_Report_NATIONWIDE-SHOWROOM_'][] = $row;
            //             }
            //         }
            ...
            //         // 3. Create ZIP
            //         $zipPath = $tmpDir . '/EDI_Allocation'.$excel_filename.'_Payroll_Report_' . $restrictedDate . '.zip';
            //         $zip = new ZipArchive();
            ...
            //         if($zone === 'LNCR Showroom' || $zone === 'VISMIN Showroom'){
            //             if(!empty($region)){
            //                 $filename = "EDI_Allocation".$excel_filename."_Payroll_Report_" . $mainzone . "_" . $region . "_" . $restrictedDate . ".xls";
            //             }else{
            //                 $filename = "EDI_Allocation".$excel_filename."_Payroll_Report_" . $mainzone . "_" . $restrictedDate . ".xls";
            //             }
            //         }else{
            //             if(!empty($region)){
            //                 $filename = "EDI_Allocation".$excel_filename."_Payroll_Report_" . $zone .  "_" . $region . "_" . $restrictedDate . ".xls";
            //             } else {
            //                 $filename = "EDI_Allocation".$excel_filename."_Payroll_Report_" . $zone . "_" . $restrictedDate . ".xls";
            //             }
            //         }
            //         header('Content-Disposition: attachment; filename="' . $filename . '"');
            //         header('Cache-Control: max-age=0');
...
<body>
    ...
    <center><h2>Payroll Allocation Report for Active Branch <span>[EDI-NEW Format]</span></h2></center>
    ...
    <script src="<?php echo $relative_path; ?>assets/js/admin/report-file/edi-allocation-format/script1.js"></script>
</body>
</html>
