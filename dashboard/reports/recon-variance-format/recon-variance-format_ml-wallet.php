<?php
    ...
    function generateDownload($conn, $database, $mainzone, $region, $date, $payrollDay) {
        ...
        $payrollquery = "SELECT
                        ...
                        LEFT JOIN " . $database[0] . ".rfp_payroll AS mc ON rm.region_code = mc.region_code AND mc.payroll_date = '$date'
                        LEFT JOIN " . $database[0] . ".payroll AS p ON rm.region_code = p.region_code AND p.payroll_date = '$date' AND p.description = 'payroll'
        ...
        if(mysqli_num_rows($payrollresult) > 0) {
            while($first_row = mysqli_fetch_assoc($payrollresult)){
                $grand_total_amount += $first_row["total_amount"];
            ...
            while ($rowdata = mysqli_fetch_assoc($payrollresult)) {
                ...
                $sheet->setCellValue('C' . $row, $rowdata['total_amount']);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['generate'])) {
        ...
        if ($mainzone) {
            ...
            // $mcashquery = "SELECT * FROM " . $database[0] . ".mcash WHERE mcash_date = '$date'";
            // $payrollquery = "SELECT      
            //             p.payroll_date,     
            //             p.mainzone,    
            //             p.region,     
    ...
    if (isset($_POST['generate']) && !empty($payrollrows)) {
        foreach ($payrollrows as $mcash) {
            $grandTotalMCash += $mcash['total_amount'];
                    ...
                    if (isset($_POST['generate'])) {
                        if (!empty($payrollrows)) {
                            foreach ($payrollrows as $mcash) {
                                ...
                                $mcashAmount = !empty($mcash['total_amount']) ? $mcash['total_amount'] : 0;
