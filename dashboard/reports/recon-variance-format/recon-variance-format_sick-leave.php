<?php
    ...
    if (isset($_POST['download'])) {
        ...
        generateDownload($conn, $database, $mainzone, $region, $restrictedDate, $endDate);
        
    }
 
    // Function to generate the download excel file
    function generateDownload($conn, $database, $mainzone, $region, $restrictedDate, $endDate) {
        $mainzone = $_SESSION['mainzone'] ?? '';
        $region = $_SESSION['region'] ?? '';
        $restrictedDate = $_SESSION['restrictedDate'] ?? '';
        $endDate = $_SESSION['endDate'] ?? '';
        ...
        $payrollDay2 = $_SESSION['payroll_day2'] ?? '';
        $payrollMonth2 = $_SESSION['payroll_month2'] ?? '';
        $payrollYear2 = $_SESSION['payroll_year2'] ?? '';
        ...
        // Determine payroll day, month and year
        if (empty($payrollDay2) || empty($payrollMonth2) || empty($payrollYear2)) {
            if (!empty($_POST['end-date'])) {
                $payrollDay2 = date('j', strtotime($_POST['end-date']));
                $payrollMonth2 = date('F', strtotime($_POST['end-date']));
                $payrollYear2 = date('Y', strtotime($_POST['end-date']));
            }
        }
        ...
        $dlsql = "SELECT 
                ...
                LEFT JOIN " . $database[0] . ".rfp_sick_leave AS mc 
                    ON rm.region_code = mc.region_code ";
                    if($restrictedDate === $endDate) {
                        $dlsql .= "AND mc.payroll_date = '$restrictedDate' ";
                    } else {
                        $dlsql .= "AND mc.payroll_date BETWEEN '$restrictedDate' AND '$endDate' ";
                    }
                $dlsql .= "LEFT JOIN " . $database[0] . ".payroll AS p 
                ...
                $dlsql .= "AND p.description = 'Sick-Leave' 
                ...
                LEFT JOIN (
                    ...
                    if($restrictedDate === $endDate) {
                        $dlsql .= "AND epr.payroll_date = '$restrictedDate' ";
                    } else {
                        $dlsql .= "AND epr.payroll_date BETWEEN '$restrictedDate' AND '$endDate' ";
                    }
                    $dlsql .= "AND epr.description = 'Sick-Leave'  
                        AND epr.remarks is null 
                        AND epr.status is null
                        AND NOT epr.description IN ('payroll', '13thMonth', 'midYearBonus')
                        AND NOT(
                ...
                LEFT JOIN (
                    ...
                    FROM " . $database[0] . ".payroll_edi_report AS epr
                    WHERE epr.ml_matic_status IN ('Inactive', 'Pending') ";
                    if($restrictedDate === $endDate) {
                        $dlsql .= "AND epr.payroll_date = '$restrictedDate' ";
                ...
                LEFT JOIN (
                    ...
                    FROM " . $database[0] . ".payroll_edi_report AS epr
                    WHERE ml_matic_status IN ('Inactive', 'Pending') ";
                    if($restrictedDate === $endDate) {
                        $dlsql .= "AND epr.payroll_date = '$restrictedDate' ";
                    } else {
                        $dlsql .= "AND epr.payroll_date BETWEEN '$restrictedDate' AND '$endDate' ";
                ...
                LEFT JOIN(
                    ...
                    WHERE ";
                    if($restrictedDate === $endDate) {
                        $dlsql .= "epr.payroll_date = '$restrictedDate' ";
                    } else {
                        $dlsql .= "epr.payroll_date BETWEEN '$restrictedDate' AND '$endDate' ";
                    }
                        $dlsql .= "AND epr.description = 'Sick-Leave'  
                        AND epr.remarks is null  
                        AND epr.status is null 
...
<body>
    ...
    <center><h2>Sick Leave Conversion Report <span>[RECON & VARIANCE-Format]</span></h2></center>
    ...
    <script>
        ...
        window.onload = function() {
            var mainzone = document.getElementById("mainzone").value;
            if (mainzone !== "") {
                updateZone(); // Fixed function call
            }
        };
    </script>
</html>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['generate'])) {
    ...
    // for Start Date
    // Determine payroll month and year
    if (!empty($_POST['restricted-date'])) {
        $payrollDay = date('j', strtotime($_POST['restricted-date']));
    ...
    if (!empty($_POST['end-date'])) {
        $payrollDay2 = date('j', strtotime($_POST['end-date']));
        $_SESSION['payroll_day2'] = $payrollDay2;
    }
    if (!empty($_POST['end-date'])) {
        $payrollMonth2 = date('F', strtotime($_POST['end-date']));
        $_SESSION['payroll_month2'] = $payrollMonth2;
    }
    if (!empty($_POST['end-date'])) {
        $payrollYear2 = date('Y', strtotime($_POST['end-date']));
        $_SESSION['payroll_year2'] = $payrollYear2;
    }
    ...
    if ($mainzone) {
        $sql = "SELECT 
                ...
                LEFT JOIN " . $database[0] . ".rfp_sick_leave AS mc 
                    ON rm.region_code = mc.region_code ";
                    if($restrictedDate === $endDate) {
                        $sql .= "AND mc.payroll_date = '$restrictedDate' ";
                    } else {
                        $sql .= "AND mc.payroll_date BETWEEN '$restrictedDate' AND '$endDate' ";
                    }
                $sql .= "LEFT JOIN " . $database[0] . ".payroll AS p 
                    ON rm.region_code = p.region_code ";
                    if($restrictedDate === $endDate) {
                ...
                $sql .= "AND p.description = 'Sick-Leave' 
                AND p.remarks is null 
                AND NOT p.description IN ('payroll', '13thMonth', 'midYearBonus')
                LEFT JOIN (
                    SELECT 
                    ...
                    WHERE epr.ml_matic_status = 'Active' ";
                    if($restrictedDate === $endDate) {
                        $sql .= "AND epr.payroll_date = '$restrictedDate' ";
                    } else {
                        $sql .= "AND epr.payroll_date BETWEEN '$restrictedDate' AND '$endDate' ";
                    }
                    $sql .= "AND epr.description = 'Sick-Leave'  
                        AND epr.remarks is null 
                        AND epr.status is null
                        AND NOT epr.description IN ('payroll', '13thMonth', 'midYearBonus')
                        AND NOT(
                ...
                LEFT JOIN(
                    ...
                    WHERE ";
                    if($restrictedDate === $endDate) {
                        $sql .= "epr.payroll_date = '$restrictedDate' ";
                    } else {
                        $sql .= "epr.payroll_date BETWEEN '$restrictedDate' AND '$endDate' ";
                    }
                        $sql .= "AND epr.description = 'Sick-Leave'  
                        AND epr.remarks is null  
                        AND epr.status is null 
                    ...
                    echo "<thead>";
                        ...
                        if($payrollYear === $payrollYear2){ // 2024 === 2024
                            if($payrollMonth === $payrollMonth2) { // 01 === 01
                                if($payrollDay === $payrollDay2){ //1===1
                                    echo "<tr>";
                                        echo "<th colspan='20'> Payroll Date: " . $payrollMonth . " " . $payrollDay .", ". $payrollYear . "</th>";
                                    echo "</tr>";
                                }else { // 1===2
                                    echo "<tr>";
                                        echo "<th colspan='20'> Payroll Date: " . $payrollMonth . " " . $payrollDay ."-" . $payrollDay2 .", ". $payrollYear . "</th>";
                                    echo "</tr>";
                                }
                            }else {
                                if($payrollDay === $payrollDay2){ //1===1
                            ...
                            if($payrollMonth === $payrollMonth2) { // 01 === 01
                                if($payrollDay === $payrollDay2){ // 1===1
                                    echo "<tr>";
                                        echo "<th colspan='20'> Payroll Date: from " . $payrollMonth . " " . $payrollDay .", ". $payrollYear . " to " . $payrollMonth . " " . $payrollDay .", ". $payrollYear2 . "</th>";
                                    echo "</tr>";
                                }else { // 1===2
                                    echo "<tr>";
                                        echo "<th colspan='20'> Payroll Date: from " . $payrollMonth . " " . $payrollDay .", ". $payrollYear . " to " . $payrollMonth . " " . $payrollDay2 .", " . $payrollYear2 . "</th>";
                                    echo "</tr>";
                                }
                            }else { // 01 === 02
                                if($payrollDay === $payrollDay2){ //1===1
                                    echo "<tr>";
                                        echo "<th colspan='20'> Payroll Date: from " . $payrollMonth . " " . $payrollDay .", " . $payrollYear . " to " . $payrollMonth2 . " " . $payrollDay .", " . $payrollYear2 . "</th>";
                                    echo "</tr>";
                                }else { // 1===2
                                    echo "<tr>";
                                        echo "<th colspan='20'> Payroll Date: from " . $payrollMonth . " " . $payrollDay .", " . $payrollYear . " to " . $payrollMonth2 . " " . $payrollDay2 .", " . $payrollYear2 . "</th>";
                                    echo "</tr>";
                                }
                            }
                        }
                    ...
                    echo "<tbody>";
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<tr>";
                                ...
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['mlwallet_amount'], 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['mlkp_amount'], 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['total_amount'], 2)) . "</td>";

                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['P_TOTAL_INCOME'], 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['P_TOTAL_DEDUCTION'], 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['P_TOTAL_NET_PAY'], 2)) . "</td>";

                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['P_VARIANCE'], 2)) . "</td>"; 

                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['EPR_TOTAL_INCOME_ACTIVE_BRANCHES'], 2)) . "</td>"; 
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['EPR_TOTAL_DEDUCTION_ACTIVE_BRANCHES'], 2)) . "</td>"; 
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['EPR_TOTAL_INCOME_ACTIVE_JEWELRY'], 2)) . "</td>"; 
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['EPR_TOTAL_DEDUCTION_ACTIVE_JEWELRY'], 2)) . "</td>";
                                // convert to negative if positive value 
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['EPR_TOTAL_INCOME_CLOSED_BRANCH'], 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['EPR_TOTAL_DEDUCTION_CLOSED_BRANCH'], 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['EPR_TOTAL_INCOME_CLOSED_JEWELRY'], 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['EPR_TOTAL_DEDUCTION_CLOSED_JEWELRY'], 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['EPR_TOTAL_CAD_NET_PAY'], 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($row['EPR_TOTAL_CAD_NET_PAY_VS_P_TOTAL_NET_PAY_VARIANCE'], 2)) . "</td>";
                            echo "</tr>";
                        }
                            echo "<tr>";
                                echo "<td colspan='3'><b>SUB-TOTAL</b></td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_mcash_wallet, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_mlkp, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_hrmd_rfp_total, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_gross_income_hrmd_edi_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_gross_deduction_hrmd_edi_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_hrmd_edi_payroll_net_pay, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_hrmd_variance_hr_rfp_vs_hr_edi_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_gross_income_active_branch_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_gross_deduction_active_branch_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_gross_income_active_jewelry_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_gross_deduction_active_jewelry_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_gross_income_closed_branch_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_gross_deduction_closed_branch_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_gross_income_closed_jewelry_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_gross_deduction_closed_jewelry_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_cad_edi_report_payroll_net_pay, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($subtotal_edi_variance_hr_edi_payroll_vs_cad_edi_report_payroll, 2)) . "</td>";
                            echo "</tr>";
                            echo "<tr>";
                                echo "<td colspan='3'><b>GRAND TOTAL</b></td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_mcash_wallet, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_mlkp, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_hrmd_rfp_total, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_gross_income_hrmd_edi_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_gross_deduction_hrmd_edi_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_hrmd_edi_payroll_net_pay, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_hrmd_variance_hr_rfp_vs_hr_edi_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_gross_income_active_branch_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_gross_deduction_active_branch_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_gross_income_active_jewelry_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_gross_deduction_active_jewelry_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_gross_income_closed_branch_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_gross_deduction_closed_branch_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_gross_income_closed_jewelry_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_gross_deduction_closed_jewelry_cad_edi_report_payroll, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_cad_edi_report_payroll_net_pay, 2)) . "</td>";
                                echo "<td style='text-align: right;'>" . htmlspecialchars(number_format($grandtotal_edi_variance_hr_edi_payroll_vs_cad_edi_report_payroll, 2)) . "</td>";
                            echo "</tr>";
                    echo "</tbody>";
                echo "</table>";
            echo "</div>";
